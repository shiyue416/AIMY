...
## GATE 0: VALIDATOR — 自动确定性验证 (Before any manual check)

运行代码级确定性验证，不通过的不进 7-Question Gate：

```bash
python aimy/tools/validator.py --vuln ssrf --url "https://target.com/fetch?url=..." --verbose
```

| 漏洞类 | 验证方法 | 确定性 |
|--------|---------|-------|
| SSRF | OOB回调、云元数据、内网端口 | ✅ **代码级** |
| SQLi | 时间盲注、布尔盲注、报错注入 | ✅ **代码级** |
| XSS | Playwright弹窗、HTTP反射 | ✅ **代码级** |
| XXE/SSTI/CMDi/LFI | OOB回调、求值验证、文件内容 | ✅ **代码级** |
| IDOR | 跨会话数据对比 | ✅ **代码级** |
| JWT/GraphQL/CORS | 算法检测/内省查询/Origin反射 | ✅ **代码级** |
| Race/Smuggling/CRLF | 部分自动+人工 | 🟡 半自动 |

**规则**: `rejected` → **立即停止，不进7问**。`downgraded` → 降级继续。
...
| 接受率 | **88%** (15/17) |
| 平均赏金 | ¥1 |
| 漏洞类型 | `improper input validation` |
