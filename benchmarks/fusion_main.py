...
def cmd_validate(args):
    """场景感知的确定性验证入口 — 按当前场景自动调整验证策略。"""
    from aimy.tools.validator import Validator
    from aimy.tools.mode import current_scene, scene_profile
    ...
    # 按场景不同输出
    if scene == "bounty":
        # 赏金模式: rejected → 立即杀, confirmed → 简洁输出
        if result.verdict == "rejected":
            out["action"] = "KILL — 不进7问门"
        elif result.verdict == "confirmed":
            out["action"] = "PROCEED — 进7问门写报告"
    elif scene == "pentest":
        # 渗透模式: 即使rejected也保留证据
        out["action"] = "LOG — 记入渗透笔记"
        if result.verdict == "confirmed":
            out["action"] = "EXPLOIT — 尝试武器化利用"
    elif scene == "redteam":
        # 红队模式: OPSEC优先，不写磁盘
        out["action"] = "OPSEC — 内存验证，不落盘"
        out["evidence"] = []  # 不输出证据细节

    if args.verbose:
        out["methods_used"] = v.METHODS.get(args.vuln, [])
...
_AUTO_VALIDATE_ENABLED = True  # 全局开关


def auto_validate_finding(vuln_class: str, url: str = "", param: str = "",
                           payload: str = "", technique: str = "") -> dict:
    """出洞自动验证 — 按当前场景策略执行。"""
    if not _AUTO_VALIDATE_ENABLED:
        return {"verdict": "needs_human", "action": "SKIP"}

    from aimy.tools.validator import Validator
    from aimy.tools.mode import current_scene
    ...
    result = v.validate(
        ...
        url=url, param=param,
        payload=payload or technique,
    )

    if scene == "bounty":
        if result.verdict == "rejected":
            return {"verdict": "rejected", "action": "KILL",
                    "reason": "确定性验证未通过，不进7问门"}
        elif result.verdict == "confirmed":
            return {"verdict": "confirmed", "action": "PROCEED",
                    "reason": "确定性验证通过，可以写报告",
                    "confidence": result.confidence}

    elif scene == "pentest":
        if result.verdict == "confirmed":
            return {"verdict": "confirmed", "action": "EXPLOIT",
                    "reason": "验证通过，尝试武器化利用",
                    "evidence": result.evidence[:3]}
        else:
            return {"verdict": result.verdict, "action": "LOG",
                    "reason": "验证未通过，但保留证据"}

    elif scene == "redteam":
        if result.verdict == "confirmed":
            return {"verdict": "confirmed", "action": "OPSEC_PROCEED",
                    "reason": "验证通过，OPSEC不落盘"}
        else:
            return {"verdict": result.verdict, "action": "OPSEC_DISCARD",
                    "reason": "验证未通过，静默丢弃"}

    return {"verdict": result.verdict, "action": "NONE"}


def inject_auto_validate(results, vuln_class: str = "", url: str = "", param: str = ""):
    """检测结果出来后过一遍确定性验证。rejected的直接丢弃(bounty)。"""
    if not results:
        return results
    ...
    validated = []
    orig_count = len(results) if isinstance(results, list) else 1
    for r in (results if isinstance(results, list) else [results]):
        if not isinstance(r, dict):
            validated.append(r)
            continue

        vc = vuln_class or r.get("vuln_class") or r.get("type", "")
        u = url or r.get("url", "")
        p = param or r.get("param", "")
        tech = r.get("technique") or r.get("payload", "")

        if vc and u:
            vr = auto_validate_finding(vc, u, p, technique=tech)
            r["_auto_verdict"] = vr["verdict"]
            r["_auto_action"] = vr["action"]

            if scene == "bounty" and vr["verdict"] == "rejected":
                continue  # 被杀掉，不留结果

        validated.append(r)

    # 打印摘要
    killed = orig_count - len(validated)
    if killed > 0:
        print(f"\n🔬 [自动验证] 杀掉 {killed}/{orig_count} 个误报, 保留 {len(validated)}/{orig_count} 个候选")
        from aimy.tools.mode import current_scene as _cs
        if _cs() != "bounty":
            print("   (非赏金模式，被杀的结果未保留)")
...
def cmd_deepscan(args):
    ...
    report = engine.run()
    # 自动验证钩子
    findings = report.get("results", report.get("findings", []))
    if findings:
        findings = inject_auto_validate(findings, url=args.target)
...
def cmd_doctor(args):
    from tools.doctor import run_all_checks, format_report
...
def cmd_suggest(args):
    ...
    from tools.schemas import SkillPhase
    tech_stack = [t.strip() for t in args.tech.split(",")] if args.tech else None
    route = suggest(
        url=args.url, goal=args.goal, phase=SkillPhase(args.phase),
        tech_stack=tech_stack, waf_detected=args.waf,
        auth_required=args.auth, has_graphql=args.graphql,
        has_api=args.api, has_jwt=args.jwt,
        has_file_upload=args.upload, veteran_mode=not args.rookie,
    )
    if args.suggest_json:
        print(route.to_json())
    else:
        print("=" * 60)
        print(f"  AIMY Skill Router - Recommendations")
        print(f"  Target: {args.url}")
        print(f"  Phase: {route.phase.value}  |  Goal: {args.goal}")
        print(f"  Inferred tech: {route.tech_stack}")
        if args.waf:
            print(f"  WAF: {args.waf}")
        print(f"  Complexity: {route.complexity}  |  Est time: {route.estimated_time_minutes} min")
        print("=" * 60)
        print(f"\n  Rationale:\n    {route.rationale}")
        print(f"\n  Recommended Skills ({len(route.recommended_skills)}):")
        for i, s in enumerate(route.recommended_skills, 1):
            print(f"    {i:2d}. {s}")
        print(f"\n  Recommended Tools ({len(route.recommended_tools)}):")
...
def cmd_mcp(args):
    from tools.mcp_bridge import bridge
    if args.mcp_command == "dashboard":
...
def main():
    ...
    p = sub.add_parser("validate", help="场景感知的确定性验证 (bounty/pentest/redteam)")
    ...
    # ---- doctor sub-command ----
    p = sub.add_parser("doctor", help="AIMY 依赖自检 (检查 Python/工具/文件/网络/MCP)")
    p.add_argument("--tools-only", action="store_true", help="仅检查外部 CLI 工具")
    p.add_argument("--json", action="store_true", help="JSON 格式输出")
    p.add_argument("--quiet", action="store_true", help="静默模式，仅返回状态码")
    p.add_argument("--network", action="store_true", help="包含网络连通性检查")
    p.set_defaults(func=cmd_doctor)

    # ---- suggest sub-command ----
    p = sub.add_parser("suggest", help="Skill 智能推荐 (输入端技术栈/目标 → 输出推荐管线)")
    p.add_argument("--url", required=True, help="目标 URL")
    p.add_argument("--goal", default="full_audit",
                   choices=["full_audit", "find_ssrf_rce", "find_sqli", "find_xss",
                           "recon_only", "quick_scan"])
    p.add_argument("--phase", default="hunt",
                   choices=["recon", "map", "hunt", "validate", "report"])
    p.add_argument("--tech", help="已知技术栈 (逗号分隔: php,laravel,nginx)")
    p.add_argument("--waf", help="WAF 类型 (cloudflare/akamai/imperva/f5/AWS)")
    p.add_argument("--auth", action="store_true", help="需要认证")
    p.add_argument("--graphql", action="store_true", help="有 GraphQL 端点")
    p.add_argument("--api", action="store_true", help="有 REST API")
    p.add_argument("--jwt", action="store_true", help="有 JWT Token")
    p.add_argument("--upload", action="store_true", help="有文件上传功能")
    p.add_argument("--rookie", action="store_true", help="菜鸟模式 (不过滤低危)")
    p.add_argument("--json", action="store_true", dest="suggest_json", help="JSON 输出")
    p.set_defaults(func=cmd_suggest)

    # ---- mcp command ----
    p = sub.add_parser("mcp", help="MCP Bridge 跨工具状态管理")
    mcp_sub = p.add_subparsers(dest="mcp_command")
    mcp_sub.add_parser("dashboard", help="显示跨工具桥接仪表盘")
    mcp_sub.add_parser("stats", help="显示事件总线统计")
